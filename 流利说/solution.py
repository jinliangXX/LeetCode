...
def get_num(s: str) -> int:
    nums, result = collections.Counter(s), 0
    nums = sorted(nums.items(), key=lambda d: d[1], reverse=True)
    trees = HuffmanTree(nums)
    trees.get_code()
    for num in nums:
        result += trees.codes[num[0]] * num[1]
    return result
